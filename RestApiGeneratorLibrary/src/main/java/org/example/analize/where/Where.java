...
import static org.example.analize.where.Where.RegExp.*;

public abstract class Where<R> extends Interpretation<R> {
    ...
    Where(String where, String table, Endpoint parent){
    ...
    Interpretation<R> makePort(String port,String table,Endpoint parent){
        ...
        while(!postfix.isEmpty()){
            ...
            if(Converter.isOperator(argument)){
                stack.push(makeOperand(stack.pop(),stack.pop(),argument));
            }
            else {
    ...
    Interpretation<R> makeFilterOrPrimitive(String port,String table,Endpoint parent){
        if(port.endsWith(RIGHT_SQUARE_BRACKET)&&port.startsWith(LEFT_SQUARE_BRACKET)){
            return makeFilter(port.substring(LEFT_SQUARE_BRACKET.length(), port.length() - RIGHT_SQUARE_BRACKET.length()),table,parent);
        }
    ...
    abstract Interpretation<R> makeFilter(String filter,String table,Endpoint parent);
    abstract Interpretation<R> makePrimitive(String primitive,String table,Endpoint parent);
    abstract Interpretation<R> makeOperand(Interpretation<R> left,Interpretation<R> right, String operand);
}
