...
public class Get extends BaseGet<CodeBlock, MethodSpec.Builder> {
    ...
    public MethodSpec.Builder createFieldsPort(MethodSpec.Builder method,boolean isFields) {
       final String parametrizedList="$T<$T<$T>>";
        final String parametrizedMap="$T<$T,$T>";
        CodeBlock defaultListBlock = CodeBlock.builder().add(parametrizedList, LIST_CLASS, FIELD_CLASS, OBJECT_CLASS).add(LIST_NAME_DEFAULT).add("=$T.of(", LIST_CLASS).add(makeFields()).add(")").build();
        method.addStatement(defaultListBlock);
        if(!isFields){
            return method;
        }
        if (isFieldsExist()) {
            CodeBlock defaultMapBlock = CodeBlock.builder().add(parametrizedMap, MAP_CLASS, STRING_CLASS, STRING_CLASS).add(FIELDS_NAME).add("=$T.of(", MAP_CLASS).add(makeFieldsName()).add(")").build();
            method.addStatement(defaultMapBlock);
            method.beginControlFlow("if(" + REQUEST_PARAM_NAME + ".containsKey($S))", FIELDS);
            CodeBlock createPort = CodeBlock.builder().add(parametrizedList, LIST_CLASS, FIELD_CLASS, OBJECT_CLASS).add(LIST_NAME).add("=").add(REQUEST_PARAM_NAME).add(".get($S).parallelStream()", FIELDS).
                    add(".filter(f->").add(FIELDS_NAME).add(".containsKey(f))").add(".map(f->").
                    add("$T.field(", DSL_CLASS).add(FIELDS_NAME).add(".get(f)).as(f)).toList()").build();
            ...
            method.beginControlFlow("if(" + REQUEST_PARAM_NAME + ".containsKey($S))", FIELDS);
            CodeBlock createPort = CodeBlock.builder().add(parametrizedList, LIST_CLASS, FIELD_CLASS, OBJECT_CLASS).add(LIST_NAME_DEFAULT).add("=")
                    .add(REQUEST_PARAM_NAME).add(".get($S).parallelStream()", FIELDS).add(".map(f->").
                    add("$T.field(", DSL_CLASS).add(FIELDS_NAME).add(".get(f)).as(f)).toList()").build();
            method.addStatement(createPort);
            method.endControlFlow();
        }
