...
@Slf4j
public abstract class BaseConditionParser<Condition, Table> implements Interpretation<Condition> {
    record PORT() {
    ...
    BaseConditionParser(String request, Variables variables, BaseTable<Table> table) {
        ...
        if (input.length == PORT.PORT_CONT) {
            ...
            Debug.debug(log, "port of comparison is ", input[BaseConditionParser.PORT.OPERATION]);
            conditionNext = makeConditionNext(input[BaseConditionParser.PORT.NEXT], variables, table);
            Debug.debug(log, "port of condition next is ", input[BaseConditionParser.PORT.NEXT]);
            return;
        }
