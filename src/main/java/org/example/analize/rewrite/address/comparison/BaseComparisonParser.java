package org.example.analize.rewrite.address.comparison;
...
@Slf4j
public abstract class BaseComparisonParser<Condition,Table,Field,Value> implements Interpretation<Condition> {
    @AllArgsConstructor
        ...
        for (Operation operation :Operation.values()) {
            ...
            if (input.length == BaseComparisonParser.PORT.PORT_CONT) {
                ...
                this.field = (!input[BaseComparisonParser.PORT.FIELD].isEmpty()) ?
                        addField(input[BaseComparisonParser.PORT.FIELD],variables,table,false) :
                        addField(input[BaseComparisonParser.PORT.VALUE],variables,table,true);
                Debug.debug(log,"field",input[BaseComparisonParser.PORT.FIELD]);
                return;
            }
