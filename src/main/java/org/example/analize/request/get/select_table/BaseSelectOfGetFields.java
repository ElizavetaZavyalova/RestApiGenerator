...
@Slf4j
public  abstract class BaseSelectOfGetFields<Select,Table,Field> extends BaseSelectFieldsParser<Select,Table,Field> {
    protected List<BaseField<Field, Table>> fields=null;
    protected BaseSelectOfGetFields(String request, Variables variables) {
    ...
    protected boolean add(String port, Variables variables) {
        ...
        if (Ident.isField(port)) {
            fields.add(addField(makeValue(port), variables));
            return true;
        }
        return false;
    }
